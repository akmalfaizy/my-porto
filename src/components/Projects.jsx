...
const projectsData = [
  {
    ...
    shortDesc:
      "Undergraduate thesis project featuring nearest attraction mapping using the Haversine formula, QR validation tickets, and culinary directories.",
    longDesc:
      "Developed using the Agile Scrum methodology, Sleman Wanderlust is a travel companion app designed for Sleman, Yogyakarta. It calculates the physical distance between the user and nearby tourist hotspots using the mathematical Haversine formula to render local recommendations. It supports QR-coded check-ins, local culinary exploration directories, real-time map navigation, bug logs, and custom accounts. This project was honored during the academic review at UTY.",
  ...
  },
  {
    ...
    tech: ["Web Development", "UI/UX Design", "Responsive", "SEO"],
    shortDesc:
      "The website of M Organizer, an active automotive event organizer in Central Sulawesi handling drag race, drag bike, modification contests, and social events.",
    longDesc: [
...
function WebProjectPlaceholder({ project }) {
  const colors = {
    4: {
      ...
      to: "to-accent-purple/20",
      text: "text-primary",
      dot: "bg-primary",
    },
    5: {
      from: "from-accent-purple/20",
      to: "to-accent-pink/20",
  ...
  return (
    <div
      className={`relative overflow-hidden aspect-[16/9] bg-slate-950/80 flex items-center justify-center bg-gradient-to-br ${c.from} ${c.to}`}
    >
      <div className="flex flex-col items-center gap-3 text-center px-4">
        <div
          className={`w-14 h-14 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center ${c.text}`}
        >
          <Globe className="w-7 h-7" />
        </div>
        <span className={`font-heading font-black text-lg ${c.text}`}>
          {project.title}
        </span>
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col mb-16 items-center text-center">
          ...
          <p className="text-slate-400 text-sm md:text-base max-w-lg mt-6">
            A curated showcase of applications and web platforms I have built
            and maintained.
          </p>
        </div>
                ...
                <div className="mt-6 pt-4 border-t border-white/5 flex flex-wrap gap-1.5 items-center justify-between">
                  <div className="flex flex-wrap gap-1">
                    {project.tech.slice(0, 3).map((t, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-0.5 rounded bg-white/5 text-slate-400 text-[9px] font-mono"
                      >
                        {t}
                      </span>
                    ))}
            ...
            >
              ...
              <div className="p-6 md:p-8 space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {selectedProject.tech.map((t, idx) => (
                      <span
                        key={idx}
                        className="px-3 py-1 rounded-lg bg-white/5 border border-white/5 text-slate-300 text-xs font-mono"
                      >
                        {t}
                      </span>
                    ))}
                ...
                <div className="space-y-4">
                  ...
                  {(Array.isArray(selectedProject.longDesc)
                    ? selectedProject.longDesc
                    : [selectedProject.longDesc]
                  ).map((paragraph, index) => (
                    <p
                      key={index}
                      className="text-slate-400 text-sm md:text-base leading-relaxed text-justify"
                    >
                      {paragraph}
                    </p>
                  ))}
